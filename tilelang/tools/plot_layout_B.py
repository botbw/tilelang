...
def plot_layout(
    ...
    font_size = 20  # Set font size for text annotation

    # For A and C layout printing
    # For sparse layout also need to update the row index labeling
    extra_path = []
    ...
    for i in range(nrows):
        for j in range(ncols):
            if i % 2 == 0:
                ...
                # E2F0D9 or white
                color = "#E2F0D9" if j % 2 == 0 else "#FFFFFF"
                # Create a rectangle patch for visualization
                rect = patches.Rectangle((j, i), 1, 2, linewidth=0.5, edgecolor="black", facecolor=color)                    
                ax.add_patch(rect)  # Add the rectangle to the plot

                if i % 8 == 0 and j % 8 == 0:
                    rect = patches.Rectangle((j, i), 8, 8, linewidth=4, edgecolor="black", facecolor="none")                    
                    extra_path.append(rect)
                ...
                ax.text(j + 0.5, i + 0.5, thread_str, ha="center", va="center", color="black", fontsize=thread_fontsize)
                ax.text(j + 0.5, i + 0.6, ":", ha="center", va="center", color="black", fontsize=thread_fontsize, rotation=90)
                ax.text(j + 0.5, i + 0.75, "}", ha="center", va="center", color="black", fontsize=thread_fontsize, rotation=90)
                ax.text(j + 0.5, i + 1.05, f"R{local_id}", ha="center", va="center", color="black", fontsize=thread_fontsize)
                ax.text(j + 0.5, i + 1.1, f",", ha="center", va="center", color="black", fontsize=thread_fontsize)
                ax.text(j + 0.5, i + 1.35, f"R{local_id + 1}", ha="center", va="center", color="black", fontsize=thread_fontsize)
                ax.text(j + 0.5, i + 1.55, "{", ha="center", va="center", color="black", fontsize=thread_fontsize, rotation=90)
    for p in extra_path:
        ax.add_patch(p)  # Add the rectangle to the plot
    
    # Add row labels to the left side of the plot
    for i in range(nrows):
        text = f"{i}"
        ax.text(-0.2, i + 0.5, text, ha="center", va="center", color="black", fontsize=font_size)

    # Add column labels at the top of the plot
    for j in range(ncols):
        text = f"{j}"
        ax.text(j + 0.5, -0.15, text, ha="center", va="center", color="black", fontsize=font_size)
    ...
    # legend_patches = [patches.Patch(color="black", label="T: Thread ID"), patches.Patch(color="black", label="R: Register ID")]
    # ax.legend(
