...
def plot_layout(
    ...
    for i in range(nrows):
        for j in range(ncols):
            if j % 2 == 0:
                ...
                # E2F0D9 or white
                color = "#E2F0D9" if i % 2 == 0 else "#FFFFFF"
                # Create a rectangle patch for visualization
                rect = patches.Rectangle((j, i), 2, 1, linewidth=0.5, edgecolor="black", facecolor=color)                    
                ...
                ax.text(j + 0.4, i + 0.5, thread_str + ":", ha="center", va="center", color="black", fontsize=thread_fontsize)
                # Add local ID text with original font size
                ax.text(j + 1.35, i + 0.5, "{" + f"R{local_id}" + f",R{local_id + 1}" + "}", ha="center", va="center", color="black", fontsize=thread_fontsize - 3)
    ...
    # for j in range(ncols):
    #     if j % 2 == 0:
    #         text = f"[{j // 2 * 4}, {j // 2 * 4 + 4})"
    #         ax.text(j + 1, -0.15, text, ha="center", va="center", color="black", fontsize=font_size)
