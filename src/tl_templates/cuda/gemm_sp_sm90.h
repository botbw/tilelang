...
namespace cute
{
    namespace tl_wgmma
    {
        template <int M, int N, int K, int num_warp_m, int num_warp_n, bool trans_A,
                  bool trans_B, bool clear_accum, typename ElementA,
                  typename ElementB, typename ElementAccumulator>
        class GemmTensorOp
        {
        public:
            static_assert(num_warp_m % 4 == 0, "num_warp_m must be a multiple of 4");

            // modified form https://github.com/NVIDIA/cutlass/blob/df8a550d3917b0e97f416b2ed8c2d786f7f686a3/include/cutlass/gemm/collective/builders/sm90_sparse_gmma_builder.inl#L85
            ...
            using ElementAMmaRaw = conditional_t<std::is_same<ElementA, float>::value,
                                         tfloat32_t, ElementA>;
            using ElementBMma = conditional_t<std::is_same<ElementB, float>::value,
                                         tfloat32_t, ElementB>;
            ...
            using TiledMma = decltype(make_tiled_mma(GMMA::ss_op_selector_sparse<
            ...
            using SparseConfig = cutlass::Sm90GemmSparseConfig<ElementAMma, GmmaMajorA, ElementEMma, decltype(min(size<2>(TileShape_MNK{}), _128{}))>;
            ...
            using SmemLayoutAtomA = decltype(detail::ss_smem_selector_sparse<
                GmmaMajorA, ElementA, decltype(get<0>(TileShape_MNK{})), decltype(get<2>(TileShape_MNK{})), ElementAMmaSparsity>());
            using SmemLayoutAtomB = decltype(detail::ss_smem_selector<
                GmmaMajorB, ElementB, decltype(get<1>(TileShape_MNK{})), decltype(get<2>(TileShape_MNK{}))>());

            // modified from https://github.com/NVIDIA/cutlass/blob/df8a550d3917b0e97f416b2ed8c2d786f7f686a3/include/cutlass/gemm/collective/sm90_sparse_mma_tma_gmma_ss_warpspecialized.hpp#L55
            ...
            static CUTE_DEVICE void body(ElementA *pA, ElementB *pB, ElementAccumulator *pC, ElementE *pE)
            {
                const int tid = threadIdx.x;
                Tensor sA = make_tensor(make_smem_ptr(pA), SmemLayoutA{});  // (BLK_M,BLK_K)
                Tensor sB = make_tensor(make_smem_ptr(pB), SmemLayoutB{});  // (BLK_N,BLK_K)
                Tensor sE = as_position_independent_swizzle_tensor(make_tensor(make_smem_ptr(pE), SmemLayoutE{}));  // (BLK_M,BLK_K)
                ...
                Tensor acc = make_tensor(make_rmem_ptr(pC), partition_shape_C(tiled_mma, Shape<Int<M>, Int<N>>{}));
                ...
                for (int k_block = 0; k_block < size<2>(tCrA); ++k_block)
                {
                    // warpgroup_arrive();
                    // (V,M) x (V,N) => (V,M,N)
                    gemm(tiled_mma, make_zip_tensor(tCrA(_, _, k_block), tCrE(_, _, k_block)), tCrB(_, _, k_block), acc);
